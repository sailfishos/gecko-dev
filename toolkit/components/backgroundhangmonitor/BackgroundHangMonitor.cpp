...
BackgroundHangManager::BackgroundHangManager()
    : mShutdown(false), mLock("BackgroundHangManager"), mIntervalNow(0) {
  // Lock so we don't race against the new monitor thread
  MonitorAutoLock autoLock(mLock);

  mHangMonitorThread =
      PR_CreateThread(PR_USER_THREAD, MonitorThread, this, PR_PRIORITY_LOW,
...
void BackgroundHangManager::RunMonitorThread() {
  ...
  while (!mShutdown) {
    ...
    if (MOZ_LIKELY(systemInterval < recheckTimeout &&
                   systemInterval >= waitTime && rv == NS_OK)) {
...
void BackgroundHangThread::ReportHang(PRIntervalTime aHangTime) {
  ...
  HangDetails hangDetails(
      ...
      VoidString(), mThreadName, mRunnableName, Move(mHangStack),
      Move(annotations));
...
bool BackgroundHangMonitor::RegisterAnnotator(
    HangMonitor::Annotator& aAnnotator) {
#ifdef MOZ_ENABLE_BACKGROUND_HANG_MONITOR

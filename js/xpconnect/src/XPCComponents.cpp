...
char* xpc::CheckAccessList(const char16_t* wideName, const char* const list[]) {
  ...
  for (const char* const* p = list; *p; p++)
    if (!strcmp(*p, asciiName.get())) return CloneAllAccess();
...
nsXPCComponents_Interfaces::Resolve(nsIXPConnectWrappedNative* wrapper,
  ...
  if (name.encodeLatin1(cx, str) && name.ptr()[0] != '{') {
    ...
    if (!info) {
      XPTInterfaceInfoManager::GetSingleton()->GetInfoForName(
          name.ptr(), getter_AddRefs(info));
    }
    ...
    if (nsid) {
      ...
      if (NS_SUCCEEDED(xpc->WrapNative(cx, obj, static_cast<nsIJSIID*>(nsid),
                                       NS_GET_IID(nsIJSIID),
                                       idobj.address()))) {
        if (idobj) {
          *resolvedp = true;
...
/***************************************************************************/
NS_IMETHODIMP
nsXPCComponents_InterfacesByID::GetInterfaces(uint32_t* aCount,
                                              nsIID*** aArray) {
  *aCount = 2;
  nsIID** array = static_cast<nsIID**>(moz_xmalloc(2 * sizeof(nsIID*)));
  *aArray = array;
...
NS_IMPL_ISUPPORTS(nsXPCComponents_InterfacesByID,
                  nsIXPCComponents_InterfacesByID, nsIXPCScriptable,
                  nsIClassInfo)
...
nsXPCComponents_InterfacesByID::Resolve(nsIXPConnectWrappedNative* wrapper,
  ...
  RootedString str(cx, JSID_TO_STRING(id));
  if (38 != JS_GetStringLength(str)) return NS_OK;

  JSAutoByteString utf8str;
  if (utf8str.encodeUtf8(cx, str)) {
    nsID iid;
    if (!iid.Parse(utf8str.ptr())) return NS_OK;

    nsCOMPtr<nsIInterfaceInfo> info;
    XPTInterfaceInfoManager::GetSingleton()->GetInfoForIID(
        &iid, getter_AddRefs(info));
    if (!info) return NS_OK;

    nsCOMPtr<nsIJSIID> nsid = nsJSIID::NewID(info);

    if (!nsid) return NS_ERROR_OUT_OF_MEMORY;

    nsXPConnect* xpc = nsXPConnect::XPConnect();
    RootedObject idobj(cx);
    if (NS_SUCCEEDED(xpc->WrapNative(cx, obj, static_cast<nsIJSIID*>(nsid),
                                     NS_GET_IID(nsIJSIID), idobj.address()))) {
      if (idobj) {
        *resolvedp = true;
        *_retval =
            JS_DefinePropertyById(cx, obj, id, idobj,
...

{
  ...
  if (JSID_IS_STRING(id) && name.encodeLatin1(cx, JSID_TO_STRING(id)) &&
    ...
    if (nsid) {
      nsXPConnect* xpc = nsXPConnect::XPConnect();
      RootedObject idobj(cx);
      if (NS_SUCCEEDED(xpc->WrapNative(cx, obj, static_cast<nsIJSCID*>(nsid),
                                       NS_GET_IID(nsIJSCID),
                                       idobj.address()))) {
        if (idobj) {
          *resolvedp = true;
...
NS_IMETHODIMP
nsXPCComponents_Classes::Initialize(nsIJSCID* cid, const char* str) {
...
class nsXPCComponents_ClassesByID final : public nsIXPCComponents_ClassesByID,
                                          public nsIXPCScriptable,
                                          public nsIClassInfo {
 public:
  // all the interface method declarations...
...
NS_IMETHODIMP
nsXPCComponents_ClassesByID::GetClassDescription(
    nsACString& aClassDescription) {
...
nsXPCComponents_ClassesByID::NewEnumerate(nsIXPConnectWrappedNative* wrapper,
  ...
  if (NS_FAILED(compMgr->EnumerateCIDs(&e)) || !e) return NS_ERROR_UNEXPECTED;
...
static bool IsRegisteredCLSID(const char* str) {
  bool registered;
  nsID id;

  if (!id.Parse(str)) return false;

  nsCOMPtr<nsIComponentRegistrar> compMgr;
  if (NS_FAILED(NS_GetComponentRegistrar(getter_AddRefs(compMgr))) ||
      !compMgr || NS_FAILED(compMgr->IsCIDRegistered(id, &registered)))
    return false;
...
nsresult nsXPCComponents_ID::CallOrConstruct(nsIXPConnectWrappedNative* wrapper,
  ...
  if (!(jsstr = ToString(cx, args[0])) || !bytes.encodeLatin1(cx, jsstr) ||
      !id.Parse(bytes.ptr())) {
  ...
  if (!newobj) return NS_ERROR_UNEXPECTED;
...
nsXPCComponents_ID::HasInstance(nsIXPConnectWrappedNative* wrapper,
  ...
  if (bp) *bp = JSValIsInterfaceOfType(cx, val, NS_GET_IID(nsIJSID));
  return NS_OK;
}
...
struct MOZ_STACK_CLASS ExceptionArgParser {
  ...
  bool parse(const CallArgs& args) {
     ...
     */
    if (args.length() > 0 && !parseMessage(args[0])) return false;
    if (args.length() > 1) {
      if (args[1].isObject()) {
        RootedObject obj(cx, &args[1].toObject());
        return parseOptionsObject(obj);
      }
      if (!parseResult(args[1])) return false;
    }
    if (args.length() > 2) {
      if (!parseStack(args[2])) return false;
    }
    if (args.length() > 3) {
      if (!parseData(args[3])) return false;
    }
    return true;
  }
 ...
 protected:
  ...
  bool getOption(HandleObject obj, const char* name, MutableHandleValue rv) {
    ...
    bool found;
    if (!JS_HasProperty(cx, obj, name, &found)) return false;
...
class nsXPCConstructor : public nsIXPCConstructor,
 ...
 public:
  ...
  nsXPCConstructor(nsIJSCID* aClassID, nsIJSIID* aInterfaceID,
                   const char* aInitializer);

 private:
  virtual ~nsXPCConstructor();
  nsresult CallOrConstruct(nsIXPConnectWrappedNative* wrapper, JSContext* cx,
                           HandleObject obj, const CallArgs& args,
                           bool* _retval);
...
nsXPCConstructor::nsXPCConstructor(nsIJSCID* aClassID, nsIJSIID* aInterfaceID,
                                   const char* aInitializer)
    : mClassID(aClassID), mInterfaceID(aInterfaceID) {
  mInitializer = aInitializer ? moz_xstrdup(aInitializer) : nullptr;
}

nsXPCConstructor::~nsXPCConstructor() {
  if (mInitializer) free(mInitializer);
}
...
NS_IMPL_ISUPPORTS(nsXPCConstructor, nsIXPCConstructor, nsIXPCScriptable,
                  nsIClassInfo)
...
NS_IMETHODIMP
nsXPCConstructor::Call(nsIXPConnectWrappedNative* wrapper, JSContext* cx,
...
NS_IMETHODIMP
nsXPCConstructor::Construct(nsIXPConnectWrappedNative* wrapper, JSContext* cx,
                            JSObject* objArg, const CallArgs& args,
                            bool* _retval) {
  RootedObject obj(cx, objArg);
  return CallOrConstruct(wrapper, cx, obj, args, _retval);
...
// static
nsresult nsXPCConstructor::CallOrConstruct(nsIXPConnectWrappedNative* wrapper,
                                           JSContext* cx, HandleObject obj,
                                           const CallArgs& args,
                                           bool* _retval) {
  nsXPConnect* xpc = nsXPConnect::XPConnect();
  ...
  RootedValue rval(cx);
  if (!JS_CallFunctionName(cx, cidObj, "createInstance",
                           JS::HandleValueArray(arg), &rval) ||
      rval.isPrimitive()) {
  ...
  if (mInitializer) {
    ...
    RootedValue fun(cx);
    if (!JS_GetProperty(cx, newObj, mInitializer, &fun) || fun.isPrimitive()) {
...
nsresult nsXPCComponents_Constructor::CallOrConstruct(
  ...
  if (args.length() >= 2) {
    ...
    if (NS_FAILED(xpc->GetWrappedNativeOfJSObject(cx, &val.toObject(),
                                                  getter_AddRefs(wn))) ||
        !wn || !(cInterfaceID = do_QueryWrappedNative(wn))) {
      return ThrowAndFail(NS_ERROR_XPC_UNEXPECTED, cx, _retval);
    }
  } else {
    nsCOMPtr<nsIInterfaceInfo> info;
  ...
  // a new scope to avoid warnings about shadowed names
  {
    ...
    if (NS_FAILED(xpc->GetWrappedNativeOfJSObject(cx, val.toObjectOrNull(),
                                                  getter_AddRefs(wn))) ||
        !wn || !(cClassID = do_QueryWrappedNative(wn))) {
      return ThrowAndFail(NS_ERROR_XPC_UNEXPECTED, cx, _retval);
    }
  }
...
nsXPCComponents_Constructor::HasInstance(nsIXPConnectWrappedNative* wrapper,
                                         ...
                                         HandleValue val, bool* bp,
                                         bool* _retval) {
  if (bp) *bp = JSValIsInterfaceOfType(cx, val, NS_GET_IID(nsIXPCConstructor));
  return NS_OK;
}
...
nsXPCComponents_Utils::GetSandbox(nsIXPCComponents_utils_Sandbox** aSandbox) {
  ...
  if (!mSandbox) mSandbox = NewSandboxConstructor();
...
nsXPCComponents_Utils::ReportError(HandleValue error, HandleValue stack,
  ...
  if (!msgstr) return NS_OK;

  nsAutoJSString msg;
  if (!msg.init(cx, msgstr)) return NS_OK;
...
NS_IMETHODIMP
nsXPCComponents_Utils::GetSandboxAddonId(HandleValue sandboxVal, JSContext* cx,
                                         MutableHandleValue rval) {
  if (!sandboxVal.isObject()) return NS_ERROR_INVALID_ARG;
...
nsXPCComponents_Utils::GetJSTestingFunctions(JSContext* cx,
  ...
  if (!obj) return NS_ERROR_XPC_JAVASCRIPT_ERROR;
...
nsXPCComponents_Utils::CreateObjectIn(HandleValue vobj, HandleValue voptions,
  ...
  if (!xpc::CreateObjectIn(cx, vobj, options, rval)) return NS_ERROR_FAILURE;
  return NS_OK;
}
...
nsXPCComponents_Utils::MakeObjectPropsNormal(HandleValue vobj, JSContext* cx) {
  ...
  // first argument must be an object
  if (vobj.isPrimitive()) return NS_ERROR_XPC_BAD_CONVERT_JS;

  RootedObject obj(cx, js::UncheckedUnwrap(&vobj.toObject()));
  JSAutoCompartment ac(cx, obj);
  Rooted<IdVector> ida(cx, IdVector(cx));
  if (!JS_Enumerate(cx, obj, &ida)) return NS_ERROR_FAILURE;
  ...
  for (size_t i = 0; i < ida.length(); ++i) {
    ...
    if (!JS_GetPropertyById(cx, obj, id, &v)) return NS_ERROR_FAILURE;

    if (v.isPrimitive()) continue;

    RootedObject propobj(cx, &v.toObject());
    // TODO Deal with non-functions.
    if (!js::IsWrapper(propobj) || !JS::IsCallable(propobj)) continue;
...
nsXPCComponents_Utils::IsDeadWrapper(HandleValue obj, bool* out) {
  *out = false;
  if (obj.isPrimitive()) return NS_ERROR_INVALID_ARG;
...
nsXPCComponents_Utils::PermitCPOWsInScope(HandleValue obj) {
  if (!obj.isObject()) return NS_ERROR_INVALID_ARG;
...
nsXPCComponents_Utils::SetWantXrays(HandleValue vscope, JSContext* cx) {
  if (!vscope.isObject()) return NS_ERROR_INVALID_ARG;
...
nsXPCComponents_Utils::GetComponentsForScope(HandleValue vscope, JSContext* cx,
  ...
  if (!scope->GetComponentsJSObject(&components)) return NS_ERROR_FAILURE;
  if (!JS_WrapObject(cx, &components)) return NS_ERROR_FAILURE;
...
nsXPCComponents_Utils::Dispatch(HandleValue runnableArg, HandleValue scope,
  ...
  if (scope.isObject()) {
    ...
    if (!scopeObj) return NS_ERROR_FAILURE;
    ac.emplace(cx, scopeObj);
    if (!JS_WrapValue(cx, &runnable)) return NS_ERROR_FAILURE;
  }

  // Get an XPCWrappedJS for |runnable|.
  if (!runnable.isObject()) return NS_ERROR_INVALID_ARG;
...
nsXPCComponents_Utils::UnwaiveXrays(HandleValue aVal, JSContext* aCx,
  ...
  if (!JS_WrapObject(aCx, &obj)) return NS_ERROR_FAILURE;
...
nsXPCComponents_Utils::GetClassName(HandleValue aObj, bool aUnwrap,
  ...
  if (!aObj.isObject()) return NS_ERROR_INVALID_ARG;
  RootedObject obj(aCx, &aObj.toObject());
  if (aUnwrap) obj = js::UncheckedUnwrap(obj, /* stopAtWindowProxy = */ false);
...
nsXPCComponents_Utils::GetIncumbentGlobal(HandleValue aCallback, JSContext* aCx,
  ...
  if (!global) {
    ...
    if (!JS_WrapValue(aCx, &globalVal)) return NS_ERROR_FAILURE;
  }
...
nsXPCComponents_Utils::GenerateXPCWrappedJS(HandleValue aObj,
  ...
  nsresult rv = nsXPCWrappedJS::GetNewOrUsed(
      obj, NS_GET_IID(nsISupports), getter_AddRefs(holder->mWrappedJS));
  holder.forget(aOut);
  return rv;
}
...
bool xpc::CloneInto(JSContext* aCx, HandleValue aValue, HandleValue aScope,
  ...
  if (!aScope.isObject()) return false;
  ...
  if (aOptions.isObject() && !options.Parse()) return false;

  {
    JSAutoCompartment ac(aCx, scope);
    aCloned.set(aValue);
    if (!StackScopedClone(aCx, options, aCloned)) return false;
  }
...
nsXPCComponents_Utils::GetWebIDLCallerPrincipal(nsIPrincipal** aResult) {
  ...
  if (!callerPrin) return NS_ERROR_NOT_AVAILABLE;
...
nsXPCComponents_Utils::GetObjectPrincipal(HandleValue val, JSContext* cx,
                                          nsIPrincipal** result) {
  if (!val.isObject()) return NS_ERROR_INVALID_ARG;
...
NS_IMETHODIMP
nsXPCComponents_Utils::GetCompartmentLocation(HandleValue val, JSContext* cx,
                                              nsACString& result) {
  if (!val.isObject()) return NS_ERROR_INVALID_ARG;
  RootedObject obj(cx, &val.toObject());
  obj = js::CheckedUnwrap(obj);
...
nsXPCComponents_Utils::AllowCPOWsInAddon(const nsACString& addonIdStr,
                                         bool allow, JSContext* cx) {
  JSAddonId* addonId = xpc::NewAddonId(cx, addonIdStr);
  if (!addonId) return NS_ERROR_FAILURE;
...
nsXPCComponents::SetReturnCode(JSContext* aCx, HandleValue aCode) {
  nsresult rv;
  if (!ToUint32(aCx, aCode, (uint32_t*)&rv)) return NS_ERROR_FAILURE;
...
NS_IMPL_CLASSINFO(nsXPCComponentsBase, &ComponentsSH::Get,
                  nsIClassInfo::DOM_OBJECT, NSXPCCOMPONENTSBASE_CID)
NS_IMPL_ISUPPORTS_CI(nsXPCComponentsBase, nsIXPCComponentsBase)

NS_IMPL_CLASSINFO(nsXPCComponents, &ComponentsSH::Get, nsIClassInfo::DOM_OBJECT,
                  NSXPCCOMPONENTS_CID)

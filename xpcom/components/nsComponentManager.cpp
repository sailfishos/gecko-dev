...
nsresult nsGetServiceFromCategory::operator()(const nsIID& aIID,
  ...
  rv = catman->GetCategoryEntry(mCategory, mEntry, getter_Copies(value));
  if (NS_FAILED(rv)) {
    goto error;
  }
  if (value.IsVoid()) {
...
void nsComponentManagerImpl::ManifestBinaryComponent(
    ManifestProcessingContext& aCx, int aLineNo, char* const* aArgv) {
  LogMessageWithContext(aCx.mFile, aLineNo,
                        "Binary XPCOM components are no longer supported.");
}

static void DoRegisterXPT(FileLocation& aFile) {
  uint32_t len;
...
void nsComponentManagerImpl::ManifestXPT(ManifestProcessingContext& aCx,
                                         int aLineNo, char* const* aArgv) {
...
void nsComponentManagerImpl::ManifestCategory(ManifestProcessingContext& aCx,
  ...
  nsCategoryManager::GetSingleton()->AddCategoryEntry(category, key, value);
}
...
bool nsComponentManagerImpl::KnownModule::EnsureLoader() {
  if (!mLoader) {
    nsCString extension;
    mFile.GetURIString(extension);
    CutExtension(extension);
    mLoader = nsComponentManagerImpl::gComponentManager->LoaderForExtension(
        extension);
  }
  return !!mLoader;
}

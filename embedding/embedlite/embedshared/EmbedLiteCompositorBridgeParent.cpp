    ...
    PlatformFrameToken* aToken)
{
  ...
  PlatformFrameToken token = {0, 0};
  {
    ...
    if (mPlatformFrameDeliveryEnabled && hasDescriptor) {
      // Keep only the newest unclaimed image. Acquisition changes Ready to
      // Acquiring under this same lock before waiting on the producer fence,
      // so images already being imported or sampled remain pinned. Retire
      // superseded Ready images here on their owning render thread.

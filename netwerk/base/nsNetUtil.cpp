...
nsresult NS_NewFileURI(nsIURI **result, nsIFile *spec,
                       nsIIOService *ioService /* = nullptr */)  // pass in
                                                                 // nsIIOService
                                                                 // to optimize
                                                                 // callers
{
...
nsresult NS_NewChannelInternal(
  ...
  rv = aIoService->NewChannelFromURIWithClientAndController(
      aUri, aLoadingNode ? aLoadingNode->AsDOMNode() : nullptr,
      aLoadingPrincipal, aTriggeringPrincipal, aLoadingClientInfo, aController,
      aSecurityFlags, aContentPolicyType, getter_AddRefs(channel));
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsresult NS_NewLocalFileStream(nsIFileStream **result, nsIFile *file,
  ...
  if (NS_SUCCEEDED(rv)) {
    ...
    if (NS_SUCCEEDED(rv)) stream.forget(result);
  }
  return rv;
}
...
class BufferWriter final : public Runnable, public nsIInputStreamCallback {
 public:
  ...
  uint64_t WrittenData() const { return mWrittenData; }
 ...
 private:
  ...
  // owning thread.
  void OperationCompleted(MonitorAutoLock &aLock, nsresult aRv) {
    mAsyncResult = aRv;
...
already_AddRefed<nsIURI> NS_TryToMakeImmutable(
    nsIURI *uri, nsresult *outRv /* = nullptr */) {
  nsresult rv;
  nsCOMPtr<nsINetUtil> util = do_GetNetUtil(&rv);
...
nsresult NS_CheckIsJavaCompatibleURLString(nsCString &urlString, bool *result) {
  *result = false;  // Default to "no"
  ...
  if (NS_FAILED(rv) || !urlParser) return NS_ERROR_FAILURE;

  bool compatible = true;
  uint32_t schemePos = 0;
  int32_t schemeLen = 0;
  urlParser->ParseURL(urlString.get(), -1, &schemePos, &schemeLen, nullptr,

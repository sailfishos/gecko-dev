...
void GLContext::AttachBuffersToFB(GLuint colorTex, GLuint colorRB,
  ...
  if (colorTex) {
    ...
    MOZ_ASSERT_IF(Renderer() != GLRenderer::AndroidEmulator,
...
void GLContext::MarkDestroyed() {
  ...
  if (!MakeCurrent()) {
    NS_WARNING(
        "MakeCurrent() failed during MarkDestroyed! Skipping GL object "
        "teardown.");
  }
...
bool GLContext::MakeCurrent(bool aForce) const {
  if (MOZ_UNLIKELY(IsDestroyed())) return false;
...
void GLContext::AfterGLCall_Debug(const char* const funcName) const {
  ...
  if (err != LOCAL_GL_NO_ERROR && !mLocalErrorScopeStack.size()) {
    printf_stderr(
        "[gl:%p] %s: Generated unexpected %s error."
        " (0x%04x)\n",

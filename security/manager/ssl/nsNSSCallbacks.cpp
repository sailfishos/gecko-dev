...
nsHTTPDownloadEvent::~nsHTTPDownloadEvent() {
  if (mResponsibleForDoneSignal && mListener) mListener->send_done_signal();
}
...
nsHTTPDownloadEvent::Run() {
  ...
  ios->NewChannel2(mRequestSession->mURL, nullptr, nullptr,
                   nullptr,  // aLoadingNode
                   nsContentUtils::GetSystemPrincipal(),
                   nullptr,  // aTriggeringPrincipal
                   nsILoadInfo::SEC_ALLOW_CROSS_ORIGIN_DATA_IS_NULL,
                   nsIContentPolicy::TYPE_OTHER, getter_AddRefs(chan));
  ...
  if (mRequestSession->mHasPostData) {
    ...
    rv = uploadChannel->SetUploadStream(uploadStream,
                                        mRequestSession->mPostContentType, -1);
    NS_ENSURE_SUCCESS(rv, rv);
  }
  ...
  rv = NS_NewStreamLoader(getter_AddRefs(mListener->mLoader), mListener);
...
mozilla::pkix::Result nsNSSHttpServerSession::createSessionFcn(
    const char* host, uint16_t portnum,
...
mozilla::pkix::Result nsNSSHttpRequestSession::createFcn(
    const nsNSSHttpServerSession* session, const char* http_protocol_variant,
    const char* path_and_query_string, const char* http_request_method,
    const OriginAttributes& origin_attributes, const TimeDuration timeout,
...
mozilla::pkix::Result nsNSSHttpRequestSession::setPostDataFcn(
    const char* http_data, const uint32_t http_data_len,
...
mozilla::pkix::Result nsNSSHttpRequestSession::trySendAndReceiveFcn(
    PRPollDesc** pPollDesc, uint16_t* http_response_code,
    const char** http_response_headers, const char** http_response_data,
  ...
  if (onSTSThread) {
    NS_ERROR(
        "nsNSSHttpRequestSession::trySendAndReceiveFcn called on socket "
  ...
  do {
    if (retry_count > 0) {
      if (retryable_error) {
        MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
                ("nsNSSHttpRequestSession::trySendAndReceiveFcn - sleeping and "
                 "retrying: %d of %d\n",
    ...
    rv = internal_send_receive_attempt(
        retryable_error, pPollDesc, http_response_code, http_response_headers,
        http_response_data, http_response_data_len);
  } while (retryable_error && retry_count < max_retries);

  if (retry_count > 1) {
    if (retryable_error)
      MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
              ("nsNSSHttpRequestSession::trySendAndReceiveFcn - still failing, "
               "giving up...\n"));
    else
      MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
              ("nsNSSHttpRequestSession::trySendAndReceiveFcn - success at "
               "attempt %d\n",
               retry_count));
  }
...
void nsNSSHttpRequestSession::AddRef() { ++mRefCount; }

void nsNSSHttpRequestSession::Release() {
...
mozilla::pkix::Result nsNSSHttpRequestSession::internal_send_receive_attempt(
  ...
  if (http_response_code) *http_response_code = mListener->mHttpResponseCode;
...
nsHTTPListener::~nsHTTPListener() {
  if (mResponsibleForDoneSignal) send_done_signal();
...
void nsHTTPListener::FreeLoadGroup(bool aCancelLoad) {
...
nsHTTPListener::OnStreamComplete(nsIStreamLoader* aLoader,
                                 ...
                                 uint32_t stringLen, const uint8_t* string) {
  mResultCode = aStatus;
  ...
  if (NS_SUCCEEDED(rv)) hchan = do_QueryInterface(req, &rv);

  if (NS_SUCCEEDED(rv)) {
    rv = hchan->GetRequestSucceeded(&mHttpRequestSucceeded);
    if (NS_FAILED(rv)) mHttpRequestSucceeded = false;
...
static char* ShowProtectedAuthPrompt(PK11SlotInfo* slot,
  ...
  if (NS_SUCCEEDED(nsrv)) {
    ...
    if (protectedAuthRunnable) {
      ...
      nsCOMPtr<nsIProtectedAuthThread> runnable =
          do_QueryInterface(protectedAuthRunnable);
      if (runnable) {
...
static void DetermineEVAndCTStatusAndSetNewCert(RefPtr<nsSSLStatus> sslStatus,
                                                PRFileDesc* fd,
